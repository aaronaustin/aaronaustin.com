...
## SEO & social sharing
`BaseLayout.astro` accepts `title`, `description`, `image`, `type`, and
`structuredData` props and renders, per page:
...
- Open Graph (`og:type`, `og:site_name`, `og:title`, `og:description`, `og:url`,
  `og:image`) and Twitter (`summary_large_image` card) tags
...
- **Release pages** (`releases/[slug].astro`) pass `description` (the release's
  ...
  `type="music.album"`, and a `MusicAlbum` JSON-LD block (`name`, `byArtist`,
  `datePublished`, `image`, `url`).
