...
- `public/favicon.svg` — linked via `<link rel="icon" type="image/svg+xml">` in
  `BaseLayout.astro`. Has an inline `<style>` with `prefers-color-scheme: dark` so
  the mark is black in light mode, white in dark mode.
