...
`image` is `/og-default.png` (in `public/`, copied from `src/img/aa-header.png`),
`type` is `"website"`.

- **Release pages** (`releases/[slug].astro`) pass `description` (the release's
  `summary`), `image` (the release `cover`, resolved to an absolute URL via
  `new URL(cover.src, Astro.site)`), `type="music.album"`, and a `MusicAlbum`
  JSON-LD block (`name`, `byArtist`, `datePublished`, `image`, `url`).
