...
- **Release pages** (`releases/[slug].astro`) pass `description` (the release's
  `summary`), `image` (the release `cover` downsized to a 1200px-wide JPEG via
  `getImage()` from `astro:assets`, resolved to an absolute URL — keeps the
  social-share fetch small instead of shipping the full-res source cover),
  `imageWidth`/`imageHeight` (from `ogImage.attributes`, since covers are square
  — 1200×1200, not 1200×630), `type="music.album"`, and a `MusicAlbum` JSON-LD
  block (`name`, `byArtist`, `datePublished`, `image`, `url`).
